...
> Archived notes from an earlier setup. Use [`../readme.md`](../readme.md)
> for the current Podman workflow; these commands are retained for history and
> may describe obsolete host requirements.
...
# common commands:
uv run -m tools.helper fetchkeys                     # update keycode_db.json from QMK
uv run -m tools.helper genkey                        # sync customKeycodes -> vial.json
uv run -m tools.helper viltokey path/to/layout.vil   # .vil save -> C LAYOUT()
uv run -m tools.helper vis --layer-names "Base,Nav,Sym,Fn" # generate keymap.svg
```
