...
> These are historical keymap notes. The current build and development
> interface is [`../readme.md`](../readme.md); current helpers live under
> `../tools/` and firmware sources under `../firmware/`.

> the sym (green) layer is bugged, homerow is shifted to the left on pic whereas in reality it starts with ! on the left
  
# Explanation of files:
- ``tools/commands.txt`` just my personal commands for compiling.
- ``tools/helpercli.py`` a python helper cli. uses below
- ``firmware/my_slovak_keymap.h`` patched version of the qmk slovak keymap that doesen't define one thing twice (soon to be merged into qmk)
...
automatically find the ``custom_keycodes`` enum in ``firmware/keymap.c`` and modifies ``firmware/vial.json`` to recognize the keys:  
...
reads a given ``.vil`` savefile and converts it into (hopefully) valid c code. some minor cleanup migh be necessary. expects you to paste it into your ``firmware/keymap.c`` 
